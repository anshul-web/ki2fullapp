...
exports.getOrders = (req, res) => {
  Order.find({ user: req.user._id }).select("_id totalAmount paymentStatus paymentType orderStatus items").populate("items.productId", "_id name productPictures").exec((error, orders) => {
    if (error) return res.status(400).json({ error });
    if (orders) {
